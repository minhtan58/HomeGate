...
#                 self._remove_device(addr)
        ...
        elif response.msg in (0x8100, 0x8102, 0x8110, 0x8401, 0x8402,
...
#             LOGGER.debug('Do nothing special for response {}'.format(response))
    ...
    def remove_device_ieee(self, ieeeaddr):
        ''' remove device '''
        zigate_ieee = self.__addr(ieeeaddr)
        data = struct.pack('!Q', zigate_ieee)
        return self.send_data(0x0026, data)
    ...
    def action_ias_enroll(self, addr, endpoint, mode = 0x00, zoneId=0):
        ...
        # mode = 2 : No Enroll Permit
        # mode = 3 : Too many Zone
        addr_mode = 2
        addr = self.__addr(addr)
        #group = self.__addr(group)
        src_endpoint = 1
    ...
    def action_ias_change_parameter_warning_device(self, ieeeaddr, duration=60, level=136):
        # level = Low :         136
        # level = medium :      137
        # level = hight:        138
        # level = veryhight:    139
        ...
        zigate_ieee = self.__addr(ieeeaddr)   
        data = struct.pack('!QBB', zigate_ieee, duration, level)
        return self.send_data(0x0405, data)

    def action_ias_add_node_to_zoneGroup(self, ieeeaddr, zoneGroup = 0):

        # zoneGroup = 0 : NONE 
        # zoneGroup = 1 : Living Room
        zigate_ieee = self.__addr(ieeeaddr)
        data = struct.pack('!QB', zigate_ieee, zoneGroup)
        return self.send_data(0x0401, data)

    def action_ias_set_zone_group_valid(self, zoneGroup = 0, status=0):
        # status = 0 : Enable valid
        # stauts = 1 : Disable valid
        
        data = struct.pack('!BB', zoneGroup, status)
        return self.send_data(0x0402, data)
        
    def action_ias_set_zone_node_valid(self, ieeeaddr, status=0):
        # status = 0 : Enable valid
        # stauts = 1 : Disable valid
        zigate_ieee = self.__addr(ieeeaddr)
        data = struct.pack('!QB', zigate_ieee, status)
        return self.send_data(0x0409, data)

    def action_get_ext_panid(self):
        '''
        action_ias_remove_device
        '''
        return self.send_data(0x0404, wait_response=0x8404)

    def action_ias_set_in_home(self):
        '''
        action for set in home
        '''
        return self.send_data(0x0406, wait_response=False)

    def action_ias_set_enable_warning(self, timedelay=30):
        '''
        action for ennable warning device
        '''
        data = struct.pack('!B', timedelay)
        return self.send_data(0x0407, data)

    def action_ias_save_node(self):
        '''
        action for ennable warning device
        '''
        return self.send_data(0x0403, wait_response=False) 
        
    def action_ias_set_diasble_warning(self):
        '''
        action for ennable warning device
        '''
        return self.send_data(0x0408, wait_response=False) 
